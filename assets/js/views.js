...
(function (w) {
  ...
  function businessDashboard(key, params) {
    ...
    view().innerHTML = `
      ...
      <div class="panel" id="pendingPanel">
        ...
        <div class="table-scroll"><table>
          <thead><tr><th>Student</th><th>Grade</th><th>Contact</th>${heads.map(k => `<th class="t-right">${U.esc(Store.HEAD_LABELS[k])}</th>`).join('')}<th class="t-right">Paid</th><th class="t-right">Pending</th><th></th></tr></thead>
    ...
    function draw() {
      ...
      $('#bizBody').innerHTML = rows.slice(0, 1000).map(r => `<tr class="clickable" data-id="${U.esc(r.s.id)}">
        ...
        || `<tr><td colspan="${5 + heads.length}" class="empty">No students pending for ${U.esc(B.name)} 🎉</td></tr>`;
    ...
    $('#bizExp').onclick = () => {
      const rows = filtered();
      const out = [['Student ID', 'Name', 'Grade', 'Father', 'Contact'].concat(heads.map(k => Store.HEAD_LABELS[k] + ' Pending')).concat(['Paid', 'Total Pending'])];
      rows.forEach(r => out.push([r.s.id, r.s.name, r.s.grade, r.s.father, r.s.contact].concat(heads.map(k => pendingHeadBal(r.s, k))).concat([bizPaid(r.s), r.bal])));
    ...
    $('#bizPrint').onclick = () => {
      ...
      const filterNote = [groupLabel ? groupLabel.replace(/^\s*—\s*/, '') : '', bizState.grade ? 'Class: ' + bizState.grade : '', bizState.head ? Store.HEAD_LABELS[bizState.head] : '', bizState.q ? 'Search: "' + bizState.q + '"' : '']
        .filter(Boolean).join(' · ');
      printPendingList(B, heads, rows, pendingHeadBal, filterNote, bizPaid);
    };
    draw();
    bindNav();
  ...
  function printPendingList(B, heads, rows, pendingHeadBal, filterNote, paidOf) {
    ...
    const colSpan = 4 + (heads.length > 1 ? heads.length : 0) + 2;
